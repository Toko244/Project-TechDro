        ...
        <div class="col-12">
            <div class="card-box">
                ...
                <div style="display: flex; align-items:center; justify-content: space-between; padding:20px 0">
                    ...
                    @if (isset($section))
                        <a href="/{{ app()->getLocale() }}/admin/componentable/create/{{ $section->id ?? '' }}/?section_id={{ $section->id }}"
                            type="button"
                            class="float-right btn btn-info waves-effect width-md waves-light">{{ trans('admin.add_component') }}</a>
                    @elseif(isset($post))
                        <a href="/{{ app()->getLocale() }}/admin/componentable/create/{{ $post->id ?? '' }}/?post_id={{ $post->id }}"
                            type="button"
                            class="float-right btn btn-info waves-effect width-md waves-light">{{ trans('admin.add_component') }}</a>
                    @endif
                </div>
                <div class="dd section-list">
                    @include('admin.componentable.list-helper', ['components' => $components ?? ''])
                </div>
    ...
    <script type="text/javascript">
        $(window).ready(function() {
            ...
            $('.btn-save-nestable').click(function() {
                var $this = $(this);
                $.post("/{{ app()->getLocale() }}/admin/componentable/arrange", {
